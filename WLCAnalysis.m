...
for i=1:L
    ...
    elseif Test(i,2)==10
        Ind(2,:)=[Test(i,1) i];
    elseif Test(i,2)==0 && Test(i,1)<150
        Ind(3,:)=[Test(i,1) i];
    else
        for j=1:length(Q)
            if abs(Test(i,2)-Q(j))<0.01
                Ind(j+3,:)=[Test(i,1) i];
            end
        end
    end
...
DATAX=Data(2:2:end,:);
DATAY=Data(1:2:end,:);
CenterBackw=[mean(DATAX(400:Ind(1,2)-10,:),1); mean(DATAY(400:Ind(1,2)-10,:),1)];
CenterForw=[mean(DATAX(Ind(1,2)+400:Ind(2,2)-10,:),1); mean(DATAY(Ind(1,2)+400:Ind(2,2)-10,:),1)];
...
for j=1:length(Q)
    j
        ...
        MSDY(j,:)=var(DataY_Rot(Ind(j+2,2)+50:Ind(j+3,2)-50,:),1);
